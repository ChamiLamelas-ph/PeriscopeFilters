case
  ...
  when [value] = 'last3d' then [column] between to_char(dateadd(day,-3,convert_timezone('America/Los_Angeles', getdate())), 'YYYY-MM-DD') and to_char(dateadd(day,-1,convert_timezone('America/Los_Angeles', getdate())), 'YYYY-MM-DD')
  -- date column should be between 6 days ago and today's date in LA
  when [value] = 'last7d' then [column] between to_char(dateadd(day,-7,convert_timezone('America/Los_Angeles', getdate())), 'YYYY-MM-DD') and to_char(dateadd(day,-1,convert_timezone('America/Los_Angeles', getdate())), 'YYYY-MM-DD')
  -- date column should be between 13 days ago and today's date in LA
  when [value] = 'last14d' then [column] between to_char(dateadd(day,-14,convert_timezone('America/Los_Angeles', getdate())), 'YYYY-MM-DD') and to_char(dateadd(day,-1,convert_timezone('America/Los_Angeles', getdate())), 'YYYY-MM-DD')
  -- date column should be between 29 days ago and today's date in LA
  when [value] = 'last30d' then [column] between to_char(dateadd(day,-30,convert_timezone('America/Los_Angeles', getdate())), 'YYYY-MM-DD') and to_char(dateadd(day,-1,convert_timezone('America/Los_Angeles', getdate())), 'YYYY-MM-DD')
